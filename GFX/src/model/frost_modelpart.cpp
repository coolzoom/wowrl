...
namespace Frost
{
    ...
    void ModelPart::SetMaterial( s_refptr<Material> pMat, const s_uint& uiSubEntity )
    {
        if (uiSubEntity.IsValid())
        {
            if (uiSubEntity >= lSubEntity_.GetSize())
            {
                Warning(CLASS_NAME, "Trying to set the material of subentity "+(uiSubEntity+1)+", but this ModelPart\n"
                    "("+pParent_->GetOgreModelName()+":"+uiID_+") only\ncontains "+lSubEntity_.GetSize()+" subentities."
                );
            }
            else
            {
