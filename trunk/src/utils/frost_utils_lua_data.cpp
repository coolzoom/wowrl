...
void Data::Set( s_ptr<Lua::State> pLua, const s_uint& uiIndex )
{
    if (mType_ == VALUE_BOOL)
        mValue_.SetB(pLua->GetBool(uiIndex));
    else if (mType_ == VALUE_FLOAT)
        mValue_.SetF(pLua->GetNumber(uiIndex));
    else if (mType_ == VALUE_DOUBLE)
        mValue_.SetD(s_double(pLua->GetNumber(uiIndex)));
    else if (mType_ == VALUE_INT)
        mValue_.SetI(s_int(pLua->GetNumber(uiIndex)));
    else if (mType_ == VALUE_UINT)
        mValue_.SetUI(s_uint(pLua->GetNumber(uiIndex)));
    else if (mType_ == VALUE_STRING)
        mValue_.SetS(pLua->GetString(uiIndex));
