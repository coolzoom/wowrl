...
{
    uiID_ = s_uint(s_float(lua_tonumber(pLua, -1)));
    ...
    if (!pParent_)
    {
        throw Exception(CLASS_NAME, "Glue missing its parent (\""+uiID_+"\") !");
    }
}
...
void LuaVirtualGlue::NotifyDeleted()
{
    pParent_ = nullptr;
}
...
LuaUIObject::LuaUIObject( lua_State* pLua ) : LuaGlue(pLua)
{
    ...
    if (!pParent_)
    {
        throw Exception(CLASS_NAME, "Glue missing its parent (\""+sName_+"\") !");
    }
}
...
void LuaUIObject::NotifyDeleted()
{
    pParent_ = nullptr;
}
...
s_bool LuaUIObject::CheckParent_()
{
    if (!pParent_)
    {
        Warning(sName_, "This widget has been deleted and can no longer be used.");
        return false;
    }
    ...
    if (mFunc.Check())
    {
        ...
        if (pArg->IsProvided())
        {
            ...
            if (pArg->GetType() == Lua::TYPE_STRING)
                pFrame = pParent_->GetManager()->GetUIObjectByName(pArg->GetString());
            else
            {
...
int LuaUIObject::_SetParent( lua_State* pLua )
{
    ...
    if (mFunc.Check())
    {
        ...
        if (pArg->IsProvided())
        {
            if (pArg->GetType() == Lua::TYPE_STRING)
            {
                pParent = pParent_->GetManager()->GetUIObjectByName(pArg->GetString());
                if (!pParent)
                    return mFunc.Return();
            }
            else
        ...
        if (pArg->IsProvided())
        {
            if (pArg->GetType() == Lua::TYPE_STRING)
            {
                ...
                if (!sParent.IsEmpty(true))
                    pParent = pParent_->GetManager()->GetUIObjectByName(sParent);
            }
            else
            {
...
int LuaUIObject::_SetRelPoint( lua_State* pLua )
{
    if (!CheckParent_())
        return 0;

    Lua::Function mFunc("UIObject:SetRelPoint", pLua);
    mFunc.Add(0, "point", Lua::TYPE_STRING);
    mFunc.Add(1, "parent name", Lua::TYPE_STRING, true);
    mFunc.Add(1, "parent", Lua::TYPE_USERDATA, true);
    mFunc.Add(2, "relative point", Lua::TYPE_STRING, true);
    ...
    if (mFunc.Check())
    {
        ...
        s_float fRelX;
        if (mFunc.IsProvided(3))
            fRelX = mFunc.Get(3)->GetNumber();

        // y
        s_float fRelY;
        if (mFunc.IsProvided(4))
            fRelY = mFunc.Get(4)->GetNumber();

        pParent_->SetRelPoint(mPoint, pParent ? pParent->GetName() : "", mParentPoint, fRelX, fRelY);
    }
