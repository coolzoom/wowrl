	...
	void Font::loadResource(Resource* res)
	{
			...
			r != mCodePointRangeList.end(); ++r)
		{
			...
			for(CodePoint cp = range.first; cp <= range.second; ++cp )
			{
				...
				std::map<CodePoint, Vector2> kerning;
				unsigned int prev, next;

                if (FT_HAS_KERNING( face ))
                {
                    FT_Vector kern;
                    for (CodePointRangeList::const_iterator r2 = mCodePointRangeList.begin();
                        r2 != mCodePointRangeList.end(); ++r2)
                    {
                        const CodePointRange& range2 = *r2;
                        for (CodePoint cp2 = range2.first; cp2 <= range2.second; ++cp2 )
                        {
                            prev = FT_Get_Char_Index( face, cp );
                            next = FT_Get_Char_Index( face, cp2 );
                            ftResult = FT_Get_Kerning( face, prev, next, FT_KERNING_DEFAULT, &kern );
                            if (ftResult)
                            {
                                LogManager::getSingleton().logMessage("Info: cannot get kerning infos for characters " +
                                    StringConverter::toString(cp) + " and " + StringConverter::toString(cp2) +
                                    " in font " + mName);
                            }
                            else
                            {
                                kerning[cp2] = Vector2((kern.x >> 6)/(Real)finalWidth, (kern.y >> 6)/(Real)finalHeight);
                            }
                        }
                    }
                }
                ...
                {
                    LogManager::getSingleton().logMessage("Info: font " + mName + " doesn't have kerning informations.");
                }
