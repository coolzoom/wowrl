    ...
    */
	...
	public:
		...
		struct GlyphInfo
		{
			...
			UVRect uvRect;
			Real aspectRatio;
			std::map<CodePoint, Vector2> kerningTable;

			GlyphInfo(CodePoint id, const UVRect& rect, Real aspect, const std::map<CodePoint, Vector2> kerning)
				: codePoint(id), uvRect(rect), aspectRatio(aspect), kerningTable(kerning)
			{
    ...
    public:
        ...
        */
        inline void setGlyphTexCoords(CodePoint id, Real u1, Real v1, Real u2, Real v2, Real textureAspect, std::map<CodePoint, Vector2> kerning)
        {
