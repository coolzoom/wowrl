...
Texture::Texture(s_ptr<GUIManager> pManager) : LayeredRegion(pManager)
{
...
Color Texture::GetVertexColor() const
{
    if (pSprite_)
...
void Texture::SetDesaturated( const s_bool& bIsDesaturated )
{
    ...
    if (bIsDesaturated_ != bIsDesaturated)
    {
        ...
        pSprite_->SetDesaturated(bIsDesaturated_);
...
void Texture::SetGradient( const Gradient& mGradient )
{
    ...
    pSprite_ = s_refptr<Sprite>(new Sprite(pManager_, Color::WHITE, 256, 256));
...
void Texture::SetTexture( s_ptr<RenderTarget> pRenderTarget )
{
    ...
    if (pRenderTarget)
    {
        pSprite_ = s_refptr<Sprite>(new Sprite(pManager_,
            pRenderTarget, s_float(pRenderTarget->GetWidth()), s_float(pRenderTarget->GetHeight())
        ));
    }
    else
    {
...
void Texture::SetColor( const Color& mColor )
{
    ...
    mColor_ = mColor;
    pSprite_ = s_refptr<Sprite>(new Sprite(pManager_, mColor_, 256, 256));

    NotifyRendererNeedRedraw();
}

void Texture::SetSprite( s_refptr<Sprite> pSprite )
{
    mGradient_ = Gradient();
    sTextureFile_ = "";
    ...
    pSprite_ = pSprite;
    SetAbsWidth(s_uint(pSprite->GetWidth()));
    SetAbsHeight(s_uint(pSprite->GetHeight()));
    lTexCoord_ = pSprite->GetTextureCoords(true);
