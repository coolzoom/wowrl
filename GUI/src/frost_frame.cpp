...
Frame::Frame(s_ptr<GUIManager> pManager) : Region(pManager), lAbsHitRectInsetList_(0), lRelHitRectInsetList_(0.0f)
{
...
void Frame::CopyFrom( s_ptr<UIObject> pObj )
{
    ...
    if (pFrame)
    {
        ...
        if (pFrame->GetFrameLevel().IsValid())
        {
            ...
            for (s_int i = 0; i < pFrame->GetFrameLevel(); ++i)
            {
        ...
        foreach (iterChild, pFrame->lChildList_)
        {
            ...
            if (!pChild->IsSpecial())
            {
                s_ptr<Frame> pNewChild = pManager_->CreateFrame(pChild->GetObjectType());
                if (pNewChild)
                {
                    ...
                    if (!pManager_->AddUIObject(pNewChild))
                    {
                        Warning(lType_.Back(),
        ...
        foreach (iterRegion, pFrame->lRegionList_)
        {
            ...
            if (!pArt->IsSpecial())
            {
                s_ptr<LayeredRegion> pNewArt = pManager_->CreateLayeredRegion(pArt->GetObjectType());
                if (pNewArt)
                {
                    ...
                    if (!pManager_->AddUIObject(pNewArt))
                    {
                        Warning(lType_.Back(),
...
void Frame::CreateTitleRegion()
{
    if (!pTitleRegion_)
    {
        ...
        if (!pManager_->AddUIObject(pTitleRegion_))
        {
            Warning(lType_.Back(),
...
void Frame::CheckPosition()
{
    ...
    {
        s_int iScreenW = s_int(pManager_->GetScreenWidth());
        s_int iScreenH = s_int(pManager_->GetScreenHeight());
...
s_ptr<LayeredRegion> Frame::CreateRegion( LayerType mLayer, const s_str& sClassName, const s_str& sName, const s_str& sInheritance )
{
    s_ptr<LayeredRegion> pRegion = pManager_->CreateLayeredRegion(sClassName);
    pRegion->SetParent(this);

    pRegion->SetDrawLayer(mLayer);
    pRegion->SetName(sName);

    if (pManager_->GetUIObjectByName(pRegion->GetName()))
    {
        Error(lType_.Back(),
            "An object with the name "+pRegion->GetName()+" already exists."
        );
        pRegion.Delete();
        return nullptr;
    }

    pManager_->AddUIObject(pRegion);
    pRegion->CreateGlue();
    ...
    if (!sInheritance.IsEmpty(true))
    {
        ...
        {
            iter->Trim(' ');
            s_ptr<UIObject> pObj = pManager_->GetUIObjectByName(*iter, true);
            if (pObj)
            {
                if (pRegion->IsObjectType(pObj->GetObjectType()))
                {
                    // Inherit from the other Region
                    pRegion->CopyFrom(pObj);
                }
                else
                {
                    Warning(lType_.Back(),
                        "\""+pRegion->GetName()+"\" ("+pRegion->GetObjectType()+") cannot inherit "
    ...
    if (pChild)
    {
        if (!lChildList_.Find(pChild->GetID()))
        {
            ...
            if (IsVisible() && pChild->IsShown())
                pChild->NotifyVisible_(!pManager_->IsLoadingUI());
            else
                pChild->NotifyInvisible_(!pManager_->IsLoadingUI());
...
const s_int& Frame::GetFrameLevel() const
{
    return iLevel_;
}
...
void Frame::DefineScript( const s_str& sScriptName, const s_str& sContent )
{
    ...
    try
    {
        pManager_->GetLua()->DoString(sStr);
    }
...
void Frame::OnEvent( const Event& mEvent )
{
    ...
    if (bIsKeyboardEnabled_ && bIsVisible_)
    {
        ...
        {
            Event mKeyEvent;
            mKeyEvent.Add(mEvent[0].Get<s_uint>().Get());
        ...
        {
            Event mKeyEvent;
            mKeyEvent.Add(mEvent[0].Get<s_uint>());
            mKeyEvent.Add(InputManager::GetSingleton()->GetKeyString((KeyCode)mEvent[0].Get<s_uint>().Get()));
...
void Frame::On( const s_str& sScriptName, s_ptr<Event> pEvent )
{
    if (lDefinedScriptList_.Find(sScriptName))
    {
            ...
            (sScriptName == "KeyUp"))
        {
            ...
            if (pEvent)
            {
                pLua->PushNumber(pEvent->Get(0)->Get<s_uint>());
                pLua->SetGlobal("arg1");
                pLua->PushString(pEvent->Get(1)->Get<s_str>());
                pLua->SetGlobal("arg2");
            }
        }
...
void Frame::SetLevel( const s_int& iLevel )
{
    if (iLevel != iLevel || !iLevel_.IsValid())
    {
        iLevel_ = iLevel;
...
void Frame::Raise()
{
    if (bIsTopLevel_)
    {
        s_int iOldLevel = iLevel_;
        iLevel_ = pManager_->GetHighestLevel(mStrata_) + 1;
        ...
        {
            s_int iAmount = iLevel_ - iOldLevel;

            s_map< s_uint, s_ptr<Frame> >::iterator iterChild;
            foreach (iterChild, lChildList_)
            {
                iterChild->second->AddLevel_(iAmount);
            }
...
void Frame::AddLevel_( const s_int& iAmount )
{
    iLevel_ += iAmount;
...
void Frame::StartMoving()
{
    if (bIsMovable_)
    {
        pManager_->StartMoving(this);
    }
}

void Frame::StopMoving()
{
    pManager_->StopMoving(this);
}
...
void Frame::StartSizing( const AnchorPoint& mPoint )
{
    if (bIsResizable_)
    {
        pManager_->StartSizing(this, mPoint);
    }
}

void Frame::StopSizing()
{
    pManager_->StopSizing(this);
}
...
LayerType Layer::GetLayerType( const s_str& sLayer )
{
    if (sLayer == "ARTWORK")
