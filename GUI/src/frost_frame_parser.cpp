...
void Frame::ParseAttributes_( s_ptr<XML::Block> pBlock )
{
    ...
    if (!sParent.IsEmpty())
    {
        if (!pParent_)
        {
            s_ptr<GUI::UIObject> pParent = pManager_->GetUIObjectByName(sParent);
    ...
    if (pBlock->IsProvided("frameLevel"))
    {
        if (!bVirtual_)
        {
            s_str sFrameLevel = pBlock->GetAttribute("frameLevel");
            if (sFrameLevel != "PARENT")
                SetLevel(s_int(sFrameLevel));
        }
        else
        {
            Warning(pBlock->GetFile()+":"+pBlock->GetLineNbr(),
                "\"frameLevel\" is not allowed for virtual widgets. Ignored."
            );
        }
    }
...
void Frame::ParseBackdropBlock_( s_ptr<XML::Block> pBlock )
{
    ...
    if (pBackdropBlock)
    {
        ...
        pBackdrop->SetBackground(pManager_->ParseFileName(
            pBackdropBlock->GetAttribute("bgFile")
        ));
        pBackdrop->SetEdge(pManager_->ParseFileName(
        ...
        s_ptr<XML::Block> pEdgeInsetsBlock = pBackdropBlock->GetBlock("EdgeInsets");
        if (pEdgeInsetsBlock)
        {
            s_ptr<XML::Block> pInsetBlock = pEdgeInsetsBlock->GetRadioBlock();
            if (pInsetBlock->GetName() == "AbsInset")
            {
                pBackdrop->SetEdgeInsets(
                    s_int(pInsetBlock->GetAttribute("left")),
                    s_int(pInsetBlock->GetAttribute("right")),
                    s_int(pInsetBlock->GetAttribute("top")),
                    s_int(pInsetBlock->GetAttribute("bottom"))
        ...
        pColorBlock = pBackdropBlock->GetBlock("EdgeColor");
        if (pColorBlock)
        {
            pBackdrop->SetEdgeColor(Color(
                s_float(pColorBlock->GetAttribute("a")),
                s_float(pColorBlock->GetAttribute("r")),
                s_float(pColorBlock->GetAttribute("g")),
                s_float(pColorBlock->GetAttribute("b"))
...
void Frame::ParseLayersBlock_( s_ptr<XML::Block> pBlock )
{
    ...
    if (pLayersBlock)
    {
        ...
        foreach_block (pLayerBlock, pLayersBlock)
        {
            ...
            {
                s_ptr<GUI::LayeredRegion> pRegion = pManager_->CreateLayeredRegion(pRegionBlock->GetName());
                if (pRegion)
                {
                    try
                    {
...
void Frame::ParseFramesBlock_( s_ptr<XML::Block> pBlock )
{
    ...
    if (pFramesBlock)
    {
        ...
        {
            s_ptr<Frame> pFrame = pManager_->CreateFrame(pElemBlock->GetName());
            if (pFrame)
            {
                try
                {
                    pFrame->SetAddOn(pManager_->GetCurrentAddOn());
