...
namespace Frost
{
    ...
    {
        XML::Document mDoc("DB/CreatureModels.xml", "DB/CreatureModels.def");
        if (mDoc.Check())
        {
            s_ptr<XML::Block> pCreatureModel;
            foreach_named_block (pCreatureModel, "CreatureModel", mDoc.GetMainBlock())
            {
                s_str sModelName = pCreatureModel->GetAttribute("name");
                s_str sFileName = pCreatureModel->GetAttribute("file");
                ModelManager::GetSingleton()->LinkModelNameToFile(sModelName, sFileName);
            }
            return true;
        }
